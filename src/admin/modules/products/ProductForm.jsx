...
import {
  ...
  StringListEditor, RowListEditor, ImageInput, PageLoading, EmptyState,
...
function SpecsEditor({ value, onChange }) {
  ...
  return (
    <div className="space-y-4">
      {value.map((group, i) => (
        <div key={i} className="rounded-xl border border-black/[.08] dark:border-white/[.08] bg-black/[.02] dark:bg-white/[.02] p-4">
          ...
          <div className="space-y-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-full border border-black/[.12] dark:border-white/12 px-3 py-1 text-[12px] font-semibold text-muted-foreground transition-all hover:border-beam/40 hover:text-foreground"
            >
...
function ImagesEditor({ value, onChange, folder }) {
  ...
  return (
    <div className="space-y-3">
      {value.map((img, i) => (
        <div key={i} className="rounded-xl border border-black/[.08] dark:border-white/[.08] bg-black/[.02] dark:bg-white/[.02] p-3.5">
          ...
          <ImageInput value={img.src} onChange={(src) => setRow(i, { src })} folder={folder} />
        </div>
      ))}
      <button
        type="button"
...
export default function ProductForm() {
  ...
  const { data: product, isSuccess, error: loadError, refetch } = useQuery({
    queryKey: [api.products.key, id],
    queryFn: () => api.products.get(id),
    enabled: isEdit,
  })

  // Seed the form once from the (possibly cached) product — never overwrite
  // in-progress edits when a background refetch lands. `isSuccess` (not
  // isFetched) so a failed fetch is treated as an error, not a 404 — the
  // service only returns null on a real 404.
  useEffect(() => {
    ...
    setForm((f) => f ?? product)
    setSlug((s) => s || product.slug)
  }, [isEdit, isSuccess, product, navigate, toast])
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_.55fr]">
        ...
        <div className="space-y-5">
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-xl border border-black/[.08] dark:border-white/[.08] bg-black/[.02] dark:bg-white/[.02] p-4">
                ...
                <p className="mt-2 text-[11.5px] leading-relaxed text-muted-foreground/70">
                  Shows the variant model grid on this family&apos;s page on the client site.
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="glass p-6 shadow-card md:p-7">
            ...
            <ImagesEditor value={form.images} onChange={(images) => set({ images })} folder={`products/${finalSlug || 'new'}`} />
          </section>
        </div>
      </div>
