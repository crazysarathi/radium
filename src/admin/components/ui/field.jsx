...
export function Toggle({ checked, onChange, label, disabled }) {
  return (
    <button
      ...
      disabled={disabled}
      onClick={(e) => {
        // A switch flip must never double as a row click — tables wire
        // onRowClick on the whole row, so keep the event here.
        e.stopPropagation()
        onChange(!checked)
      }}
      className={cn(
    ...
    >
      <span
        className={cn(
          ...
          checked ? 'border-beam/60 bg-beam/80 shadow-glow' : 'border-black/15 dark:border-white/15 bg-black/[.06] dark:bg-white/[.06]'
        )}
      >
        <span
          className={cn(
