          ...
          {value.map((item, i) => (
            ...
            >
              ...
              <button
                ...
                className="rounded-full p-1 text-muted-foreground transition-colors hover:bg-black/[.06] dark:hover:bg-white/10 hover:text-foreground"
...
export function RowListEditor({ value = [], onChange, fields, addLabel = 'Add row', blank, reorder = true }) {
  ...
  return (
    <div className="space-y-2.5">
      {value.map((row, i) => (
        <div key={i} className="flex items-start gap-2 rounded-xl border border-black/[.08] dark:border-white/[.08] bg-black/[.02] dark:bg-white/[.02] p-2.5">
          {reorder ? (
          ...
          <div className="flex shrink-0 flex-col gap-1 pt-0.5">
            {reorder ? (
              <>
                <button type="button" onClick={() => move(i, -1)} disabled={i === 0} className="rounded-md p-1 text-muted-foreground transition-colors hover:bg-black/5 dark:hover:bg-white/5 hover:text-foreground disabled:opacity-25" aria-label="Move up">
                  <ArrowUp className="h-3.5 w-3.5" />
                </button>
                <button type="button" onClick={() => move(i, 1)} disabled={i === value.length - 1} className="rounded-md p-1 text-muted-foreground transition-colors hover:bg-black/5 dark:hover:bg-white/5 hover:text-foreground disabled:opacity-25" aria-label="Move down">
